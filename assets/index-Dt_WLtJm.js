...
<svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" class="size-6">
  <path stroke-linecap="round" stroke-linejoin="round" d="m12.75 15 3-3m0 0-3-3m3 3h-7.5M21 12a9 9 0 1 1-18 0 9 9 0 0 1 18 0Z" />
    ...
    `,"/work":`
      ...
      <contact-component></contact-component>
    `,"/contact":"<contact-component></contact-component>","/profile/:id":s=>`<profile-component user-id="${s.id}"></profile-component>`},t=window.location.pathname,n=document.querySelector("main");for(const s in e){const o=new RegExp(`^${s.replace(/:\w+/g,"(\\w+)")}$`),i=t.match(o);if(i){const a=(s.match(/:(\w+)/g)||[]).map(c=>c.substring(1)),d=Object.fromEntries(a.map((c,p)=>[c,i[p+1]]));n.innerHTML=typeof e[s]=="function"?e[s](d):e[s],window.scrollTo({top:0,behavior:"smooth"});return}}n.innerHTML="<error-component></error-component>"}document.addEventListener("DOMContentLoaded",()=>{l(),window.addEventListener("popstate",l)});document.body.addEventListener("click",e=>{e.target.matches("a[href]:not([target])")&&(e.preventDefault(),window.history.pushState({},"","/yuliapsavinkova.github.io/"+e.target.href),l())});
